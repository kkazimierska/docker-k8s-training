...
```bash
$ docker run --name static-site-2 -e AUTHOR="Your Name" -d -p 8888:80 static-site
```
...
Docker images are the basis of containers. To see the list of images that are available locally on your system, run the `docker images` command.
...
If you don't have the `alpine:3.5` image, the client will first pull the image and then create your image. Therefore, your output on running the command will look different from mine. If everything went well, your image should be ready! Run `docker images` and see if your image (`myfirstapp`) shows.
...
## Exercises
1. Add a new template called "dogs.html" in the templates directory and display random gifs of dogs under the `/dogs` url. Remember to update the docker file to include the new template.

## Next Steps
For the next step in the tutorial head over to [3.0 Deploying an app using compose](./votingapp.md)
